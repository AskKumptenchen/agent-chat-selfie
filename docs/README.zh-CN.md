...
- ​情感可视化：根据对话上下文，智能体既可以实时生成最贴合当下语境的情感自拍，也可以在低成本模式下直接复用与 mood 绑定的固定表情包图片。
...
如果每轮实时生图的成本过高，Chat Selfie 也支持切换到固定表情包模式。在这个模式下，运行时仍会先判断当前 mood，但不会再生成新图，而是直接读取该 mood 对应的本地图片资源，并通过当前启用的发送路径把图片和回复文本一起发给用户。
